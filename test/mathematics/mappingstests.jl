@testitem "Mappings" setup=[Validate] begin
    ...
    @test Validate.validate(o1, atol=1e-8)
    ...
    s = Sin(0 .. 2π)
    c = Cos(0 .. 2π)
    @test Validate.validate(s)
    @test Validate.validate(antiderivative(s))
    @test Validate.validate(c)
    @test Validate.validate(antiderivative(c))
    ...
    f = Exp(0 .. 2π)
    @test Validate.validate(f)
    @test Validate.validate(antiderivative(f))
    @test exp(x) == Exp(0 .. 2)
    ...
    id = Identity(0.0 .. 5.0)
    @test id(3) == 3
    @test Validate.validate(id, atol=1e-6)
    ...
    @test Validate.validate(id, atol=1e-7)
    ...
    @test Validate.validate(u, atol=1e-7)
    ...
    @test degree(p) == 5
    @test Validate.validate(p)
    @test Validate.validate(antiderivative(p))
    ...
    for l ∈ l4
        @test Validate.validate(l, atol=1e-4)
    end
    for i ∈ 1:5, j ∈ 1:5
    ...
    @test Validate.validate(2 * m1, rtol=1e-4)
    ...
    @test Validate.validate(m1 + m3, rtol=1e-4)##
    ...
    @test Validate.validate(f + 2)
    @test Validate.validate(2 + f)
    @test Validate.validate(f - 2)
    @test Validate.validate(2 - f)
    ...
    @test Validate.validate(m5, rtol=1e-4)
    ...
    @test Validate.validate(sin(x) * cos(x), rtol=1e-4)

    # Quotient
    p = m1 / m3
    @test p(0.2) ≈ tan(0.2)
    @test Validate.validate(p, rtol=1e-4)
    f = 2 / Polynomial(0, 1)
    @test f(0.2) ≈ 10
    @test pois(f) == [0]
